...
from factories.article import make_db_article
...
async def test_fetch_articles_endpoint(
    client,
    mock_article_repository,
):
    mock_article_repository.get_stale_tags.return_value = []
...
async def test_get_article_by_id_success(
    ...
    response = await client.get(f"/api/articles/{doc['_id']}")
...
async def test_get_article_not_found_returns_404(
    ...
    response = await client.get(f"/api/articles/{ObjectId()}")

    assert response.status_code == 404


async def test_get_article_invalid_id_returns_400(client):
    response = await client.get("/api/articles/not-a-valid-id")
